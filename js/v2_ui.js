...
window.V2UI = (() => {
    ...
    const paths = {
        ...
        code: 'M9 6l-6 6 6 6M15 6l6 6-6 6', point: 'M19 12a7 7 0 1 1-14 0 7 7 0 0 1 14 0'
    };
    function icon(name) { return `<svg class="v2-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="${paths[name] || paths.info}"/></svg>`; }
    function icons() {
        const map = [['.back-btn-game span:first-child','back'],['#toggle-blockly-btn .ui-icon','code'],['#mission-briefing-btn .ui-icon','info'],['#camera-mode-btn .ui-icon','camera'],['#utility-menu-btn>span:first-child','more'],['#run-blockly-btn>span:first-child','play'],['#debug-pause-btn>span:first-child','pause'],['[onclick="stepExecution()"]>span:first-child','step'],['.program-btn--stop>span:first-child','stop'],['.program-btn[onclick="resetSimulator()"]>span:first-child','reset'],['.breakpoint-dot','point'],['.blockly-tool-btn--import .blockly-tool-btn__glyph','upload'],['.blockly-tool-btn--export .blockly-tool-btn__glyph','download']];
    ...
    function camera(mode) {
        ...
        camRadius=followDrone?(currentGameMode==='freeplay'?FOLLOW_CAMERA_RADIUS:220):(currentGameMode==='freeplay'?FREE_CAMERA_RADIUS:3000);
    ...
    function programEnded() {
        text('v2-action-status',state.missionCompleted?'任務完成':state.stopSignal?'已停止':'程式執行完畢');
        text('v2-action-label',state.missionCompleted?'救援目標已達成。':state.stopSignal?'調整積木後，可以重設再試。':'觀察飛行結果，再試一個新想法。');
    }
    function sync() {
        ...
        text('hud-flight-state',state.collisionDetected?'碰到障礙物':state.isFlying?'飛行中':'已降落');
    ...
    icons();
    return {enter,setView,workspaceReady,starter,undo,camera,command,programEnded,sync,briefing,nextMission,toggleTelemetry,toggleDebug,icon,resetFeedback,projectLabels};
})();
