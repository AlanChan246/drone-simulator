...
- Fire A's existing always-on-top label occupies approximately the default drone height and hides it during release. The promo's actual Blockly program adds an existing UP 110 cm command after takeoff, resulting in 204 cm flight height (within the charge ceiling of 220 and below the obstacle bypass threshold of 420). This is an authored flight program, not a mission/physics change. The final 43-command route completed all four fires and landing in 80 browser-clock seconds for 1,225 points, with no collision or page errors; captures/mission2-proof.json records the actual XML and result.
